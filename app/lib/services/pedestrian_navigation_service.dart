...
class PedestrianNavigationService {
  ...
  List<NavigationInstruction> _generateInstructions(
    ...
    for (var i = 0; i < routePath.length - 1; i++) {
      ...
      // Detectar obstáculos (simplificado - en producción usar datos OSM)
      ObstacleType? obstacle;
      if (knownObstacles != null && knownObstacles.isNotEmpty) {
        // Asignar obstáculos aleatoriamente para demo
        if (i % 5 == 0 && i < knownObstacles.length) {
          obstacle = knownObstacles[i % knownObstacles.length];
        }
      }
  ...
  String _getStreetName(int index) {
    // Placeholder - en producción obtener de datos OSM
    final streets = [
