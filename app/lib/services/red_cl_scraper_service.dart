...
class RedClScraperService {
  ...
  Future<List<BusInfo>> getBusInfoForStop(String stopId) async {
    try {
      ...
      if (response.statusCode != 200) {
        throw Exception('Error al obtener datos de red.cl: ${response.statusCode}');
      }
  ...
  List<BusInfo> _parseHtmlResponse(String html) {
    ...
    try {
      // Buscar patrones en el HTML para extraer información de buses
      // Patron típico: <div class="bus-info">...
      
      // Por simplicidad, vamos a simular el parsing con regex básico
      // En producción se debería usar un parser HTML como html/dom
      ...
      RegExp destPattern = RegExp(r'(?:hacia|->|a)\s*([^<>\n]+)', caseSensitive: false);
      ...
      for (RegExpMatch match in matches) {
        ...
        if (route.isNotEmpty) {
          buses.add(BusInfo(
            ...
            destination: destination.isNotEmpty ? destination : 'Destino no disponible',
  ...
  RouteDetails _parseRouteDetails(String html, String routeNumber) {
    // Implementación básica - en producción se debería usar un parser HTML completo
    return RouteDetails(
